...
For the second example of logistic regression, the Fisher information based on the marginal and the expected conditional precision are:
 \be
\mc I(\theta) &=  X' \diag\bigg\{\frac{\exp(\xtheta)}{ \{1+\exp(\xtheta)\} ^2}\bigg\} X, \quad \ \bb E_z \big( \mbox{var}_{r,b} (\theta \mid z) \big)^{-1}= X'  \diag\bigg\{ \frac{r_i}{2 |\xtheta+b_i|}\tanh\Big(\frac{|\xtheta+b_i|}{2} \Big)\bigg\} X 
\ee
