...
\be
1 \wedge \prod_i \frac{   L_{r,b}(\xtheta;y_i) L(\xtheta^*;y_i)}{  L_{r,b}(\xtheta^*;y_i)L(\xtheta;y_i) } =1 \wedge \prod_i  \frac{ \{1+\exp(\xtheta)\}   \{1+\exp(\xtheta^*+b_i)\}^{r_i} } {  \{1+\exp(\xtheta^*)\}  \{1+\exp(\xtheta+b_i)\}^{r_i}    },
\ee
...
%LD: adding the second distance used in logistic and poisson tuning.
...
% \be
% \Delta_{\mc I^{-1}}(\hat{\theta}_{MAP}) &= \|  \mc I^{-1}(\hat{\theta}_{MAP})- \bb E_z \mbox{var}_{r,b} (\theta \mid z) \|_F, \\
