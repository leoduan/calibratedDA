...
We apply CDA to an online browsing activity data. The dataset contains a two-way count table of users who visited one of $96$ client websites of interests, and one of the  $n=59,792$ high-traffic sites during the same browsing session. For simplicity, we name this behavior as ``co-browsing''. For each of the client websites, it is of large commercial interests to find out the high-traffic sites with relatively high co-browsing rates, so that the ads can be more effectively placed. For the ads company, it is also useful to understand the correlation and predict the traffic patterns among the clients. Therefore, We now consider two models in this section.
...
We first illustrate CDA in the scenario when there are large number of the random effects that requires the data augmentation for their posterior sampling. We focus on one client website and analyze its co-browsing rates with the high-traffic ones. With the total visit count $N_i$ available for each high-traffic site, the count of co-browsing $y_i$ can be considered as the result of binomial trials. As the rates are often quite small ($0.00011 \pm  0.00093$) due to the dramatic difference between $y_i$ and $N_i$, using empirical ratio ${y_i}/{N_i}$ can lead to unrealistic estimate. For example, when $y_i=0$, estimating the rate as exactly $0$ is not ideal since it is merely a result of extremely low probability. Therefore, it is useful to consider a hierarchical model to allow borrowing of information across outcomes.
...
\ee
Based on expert opinion in quantitative advertising, we use a weakly informative prior $\theta_0\sim \No(-12,49)$ and non-informative prior on $\pi(\sigma^2_0)= 1/\sigma^2_0$. Similar to the logistic regression, we calibrate the binomial Polya-Gamma augmentation, leading to the proposal likelihood:
...
\ee
and accepted / rejected using the M-H step. Since the $\theta_i$'s are conditionally independent, the calibrated proposal can be individually accepted with high probability for each $i$. This leads to a high average acceptance of $0.9$, despite of the high dimensionality $59,792$ of $\theta_i$'s.

Figure~\ref{data_binomial} shows the boxplots of the ACFs for all $\theta_i$'s. We compare the result with the original DA \citep{polson2013bayesian}. We run each algorithm for $4,000$ steps and use the last $1,000$ as the posterior sample. All of the parameters mixes poorly in DA, caused by the small binomial probability; CDA leads to significant improvement that the autocorrelation rapidly decays to ignorable level within $5$ lags. Table~\ref{tab:binomial} lists the posterior mean and standard deviation for $\theta_0$ and $\sigma_0^2$, as well as the computing time per every $1,000$ steps. To provide a reference, we ran Hamiltonian Monte Carlo (HMC) provided by the Stan software \citep{carpenter2016stan}. HMC enjoys good mixing performance but is computationally intensive due to the numeric leapfrog steps. The parameter estimates from CDA and HMC are remarkably close, while slow mixing in the original DA caused poor estimates.
 ...
 \end{subfigure} 
 \caption{Boxplots of the ACFs show the mixing of the $59,792$ parameters in the hierarchical binomial model, for the original DA\citep{polson2013bayesian} and the CDA.}
 \label{data_binomial}
 \end{figure}
...
DIC  &      $3.6747 \times 10^7$  & $3.6663 \times 10^7$       & $3.6663 \times 10^7$\\
