...
It is easy to obtain good acceptance rate in CDA, because the increase in conditional variance does not affect  the existence of marginal closed-form, and the calibrated marginal is close to the original form. This is because of the data augmentation form $\pi(\theta|y)=\int f(\theta|z,y)\pi(z|y) dz$.

 In the previous example, since $\mbox{var}(\theta|z,y)$ does not involve the random latent variable $z$, one can directly use $r$ on it to increase the conditional variance. On the other hand, when $\mbox{var}(\theta|z,y)$ depends on the random value of $z$, including $r$ in it is effective and  could lead to integration difficulty. Therefore, we instead modify $\pi(z|y)$ to stochastically influence the the value of $z$. 
...
Instead, observing the value of $z$ can be influenced by the first parameter in $\PG(a_1,a_2)$, $\bb{E}z_i= \frac{a_1}{2 a_2}\tanh(\frac{a_2}{2})$, we replace $\PG(z_i \mid 1,0)$ with $\PG(z_i \mid r_i,0)$. It can be verified the integration is still tractable, applying the mean adjusting term $b_i$ on $\xbeta$, leading to:
...
Smaller $r_i$ induces larger $(X' Z X)^{-1}$ with large probability. Formally, combining \cite{cressie1981moment} and \cite{polson2013bayesian}, the first negative moment can be derived as $\bb{E}z_i^{-1}= \int_0^{\infty} \prod_{k=1}^{\infty} (1+ d_k^{-1} t) ^{-r_i} dt$ with $d_k=2(k-\frac{1}{2})^2\pi^2 + \frac{(x_i\theta+b_i)^2}{2}$, the conditional variance is increased in its expectation $\bb E (X' Z X)^{-1}$ with decreased $r_i$'s.

For choosing $r$ during tuning, we compare the two Fisher information matrices based on the marginal and conditional; for the latter, since it depends on $z_i$, we marginalize it over by taking its expectation.
...
For numerical illustration, we use a two parameter intercept-slope model with $x_1\sim \No(0,1)$ and $\theta=\{-9,1\}$. With $n= 10^5$, it leads to rare positive outcome $\sum y_{i} = 50 $. We adapt the $r$ and $b$ for $100$ steps, reaching an acceptance rate of $0.8$; then stop adaption and run $100$ and $500$ steps for burn-in and the posterior collecting.

The different mixing performances in the original DA and the calibrated one are shown in Figure~\ref{logit_random_mixing}.
...
\end{subfigure}
 \caption{Panel (a) demonstrates in traceplot and panel (b) in autocorrelation about the substantial improvement in CDA by correcting the variance mis-match in logistic regression with rare event data, compared with the original DA \citep{polson2013bayesian}.}
...
\prod_i L(m_i(\theta);y_i) =\prod_i  C_i \int \pi\left(m_i(\theta)|z_i,y_i \right)\pi(z_i|y_i) d z_i
\ee
where  $m_i(\theta)$ is the function $m_i:\bb R^p \mapsto \bb R$, conditioning on which, each integral on the right hand side is independent. For example,  $m_i (\theta) = x_i\theta$ is the linear predictor in regression; $C_i$ is the constant free from $\theta$. Let the the conditional distribution for $z$ and $\theta$ be:
...
\prod_i L_r(m_i(\theta) ; y_i,  r_i, b_i) = \prod_i C_{r,i}\int \pi_r\left(m_i(\theta)+b_i|z_i,y_i \right)\pi_r(z_i|y_i) d z_i
\ee

The proposal can then be sampled from the calibrated distribution:

\be
z_i & \sim \pi(z_i; m_i(\theta)+b_i, y) \\
\theta^* &\sim f( \theta^*|\mu(r_i, b_i),\Sigma(r_i)).
\ee
...
We initially run the algorithm by adaptively estimating $r$ and $b$. To choose the value for $r$, we first compute the Fisher information matrix based on $L(m_i(\theta);y_i)$:

\be
I(\theta) = \left [  \bb E_y  \left( \frac { \partial \log L( \theta)} {\partial \theta_{j_1}} \right)  \left( \frac { \partial \log  L(\theta)} {\partial \theta_{j_2}} \right) \right ]_{j_1,j_2}
= J' \left [ \bb E_y \left( \frac { \partial \log L( \theta)} {\partial m_{i_1}(\theta)} \right)  \left( \frac { \partial \log  L(\theta)} {\partial m_{i_2}(\theta)} \right) \right]_{i_1,i_2} J,
\ee
where $J_{i,j} = \frac {\partial m_{i}(\theta)}  { \partial \theta_j}$; then the Fisher information matrix based on $ f( \theta;\mu(b_i, z),\Sigma(z, r_i))$, marginalized over $z$:

\be
\bb E_z I(\theta| z)= \bb E_z \left [  \bb E_y  \left( \frac { \partial \log f( \theta; \mu, \Sigma)} {\partial \theta_{j_1}} \right)  \left( \frac { \partial \log  f(\theta; \mu, \Sigma)} {\partial \theta_{j_2}} \right) \right ]_{j_1,j_2}
= J' \left [ \bb E_z \bb E_y \left( \frac { \partial \log f( \theta; \mu, \Sigma)} {\partial m_{i_1}(\theta)} \right)  \left( \frac { \partial \log  f(\theta; \mu, \Sigma)} {\partial m_{i_2}(\theta)} \right) \right]_{i_1,i_2} J.
\ee

Since $r_i$'s are parameters in $\bb E_z I(\theta| z)$, we set them to the values that minimizes the difference from $I(\theta)$. Due to the conditional independence induced by $m_i(\theta)$,  computing  $r_i$ is often straightforward. Then conditional on $r_i$, we choose $b_i$ to optimize the acceptance rate, one choice is the solution to $L_r(m_i(\theta); y_i,  r_i, b_i)=L(m_i(\theta); y_i)$. After the acceptance rate reaches satisfactory value, we stop tuning $r$ and $b$, and continue in burn-in and posterior sample collecting.
