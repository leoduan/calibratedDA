...
poissonCDA<- function(y,X, r_ini= 1,tune= 100,burnin=500, run=500 ,fixR = FALSE, randomEffect= FALSE,sigma2= 0.1){
  ...
  for(i in 1: (burnin+run)){
    ...
    if(tuning){
      ...
      if(!fixR){
        ...
        r[Xbeta> -2]<- exp(Xbeta[Xbeta> -2])*20
      }
    ...
    if(randomEffect){
      ...
      sigma2<- 1/rgamma(1, n/2, rate= sum(eta^2)/2)
    }
  ...
  return(list("beta"=trace_beta, "proposal"=trace_proposal , "r"=r, "b"=b, "accept_rate"= accept/run, "eta"=trace_eta, "sigma2"=trace_sigma2))
